...
class data_export:
    def __init__(self):
        ...
        self.jackal_sub = rospy.Subscriber('/accel/turtlebot3', Twist, self.turtlebot3_callback)
    ...
    def turtlebot3_callback(self,data):
        x = data.linear.x
        y = data.linear.y
        z = data.linear.z
        roll = data.angular.x
        pitch = data.angular.y
        yaw = data.angular.z
        turtlebot3.append([x,y,z,roll,pitch,yaw])
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if key == 's':
            ...
            write_csv(jackal,"turtlebot3.csv")
            print "Data Saved"
        if key == 'q':
            quit()
