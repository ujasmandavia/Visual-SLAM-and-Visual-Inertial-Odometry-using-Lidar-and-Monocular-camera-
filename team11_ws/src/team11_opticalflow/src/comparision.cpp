...
private:
  ...
  ros::Subscriber turtlebot3_odometry_sub;
...
public:
  ...
  geometry_msgs::Pose2D turtlebot3_pose;
  ...
  geometry_msgs::Twist turtlebot3_twist;
  ...
  geometry_msgs::Twist turtlebot3_twist_prev;
...
public:
  DataGrab(ros::NodeHandle n) // CONSTRUCTOR
  {
    ...
    turtlebot3_odometry_sub = nh.subscribe("/odometry/filtered", 1, &DataGrab::turtlebot3_callback, this);

    // initialize times:
    t_prev_imu_odo = ros::Time::now().toSec();
    t_prev_imu_data = ros::Time::now().toSec();
    t_prev_visual = ros::Time::now().toSec();
    t_prev_turtlebot3 = ros::Time::now().toSec();
...
private:
  ...
  void turtlebot3_callback(const nav_msgs::Odometry::ConstPtr& msg)
  {
    double t_curr = ros::Time::now().toSec();
    double dt = t_curr - t_prev_turtlebot3;
    ...
    // and store values in Pose2D
    turtlebot3_pose.x = msg->pose.pose.position.x;
    turtlebot3_pose.y = msg->pose.pose.position.y;
    turtlebot3_pose.theta = Y;

    // extract twist data
    turtlebot3_twist.linear.x = msg->twist.twist.linear.x;
    turtlebot3_twist.linear.y = msg->twist.twist.linear.y;
    turtlebot3_twist.linear.z = msg->twist.twist.linear.z;
    turtlebot3_twist.angular.x = msg->twist.twist.angular.x;
    turtlebot3_twist.angular.y = msg->twist.twist.angular.y;
    turtlebot3_twist.angular.z = msg->twist.twist.angular.z;

    // calculate accel data
    turtlebot3_accel.linear.x = (turtlebot3_twist.linear.x - turtlebot3_twist_prev.linear.x) / dt;
    turtlebot3_accel.linear.y = (turtlebot3_twist.linear.y - turtlebot3_twist_prev.linear.y) / dt;
    turtlebot3_accel.linear.z = (turtlebot3_twist.linear.z - turtlebot3_twist_prev.linear.z) / dt;
    turtlebot3_accel.angular.x = (turtlebot3_twist.angular.x - turtlebot3_twist_prev.angular.x) / dt;
    turtlebot3_accel.angular.y = (turtlebot3_twist.angular.y - turtlebot3_twist_prev.angular.y) / dt;
    turtlebot3_accel.angular.z = (turtlebot3_twist.angular.z - turtlebot3_twist_prev.angular.z) / dt;

    turtlebot3_twist_prev = turtlebot3_twist; // for next round

    t_prev_turtlebot3 = t_curr; // update time for next call

  } // END OF turtlebot3_callback() FUNCTION
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher turtlebot3_pub = nh.advertise<geometry_msgs::Twist>("/accel/turtlebot3", 1);
  ...
  while(ros::ok())
  {
    ...
    //   cout << endl;
    //   cout << setw(3 * WIDTH) << " TURTLEBOT3:" << setw(3 * WIDTH) << "VISUAL:" << setw(3 * WIDTH) << "IMU:" << endl;
    //   for(int i = 0; i < 3; ++i)
    ...
    // print out pose info:
    // cout << setw(WIDTH) << c.turtlebot3_pose.x << setw(WIDTH) << c.turtlebot3_pose.y << setw(WIDTH) << c.turtlebot3_pose.theta;
    ...
    // cout << setw(WIDTH) << c.turtlebot3_accel.linear.x << setw(WIDTH) << c.turtlebot3_accel.angular.z;
    ...
    turtlebot3_pub.publish(c.turtlebot3_accel);
